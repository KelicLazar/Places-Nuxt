...
import { CURRENT_LOCATION_PAGES, CURRENT_LOG_LOCATION_PAGES, LOCATION_PAGES } from "~/lib/constants";
...
export const useLocationStore = defineStore("useLocationStore", () => {
  ...
  const locationLogUrlWithSlugAndId = computed(() => {
    return `/api/locations/${route.params.slug}/${route.params.id}`;
  });
  ...
  effect(() => {
    ...
    else if (currentLocationLog.value && CURRENT_LOG_LOCATION_PAGES.has(route.name?.toString() || "")) {
